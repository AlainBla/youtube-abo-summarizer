...
`--hours` and `--all` are mutually exclusive. `--limit` keeps only the N newest videos after all other filters are applied (`0` = no limit); it defaults to `100` so an unqualified `--all` still produces a reasonably sized book. The default output filename is `ebook_YYYY-MM-DD_HH-MM.epub`.
