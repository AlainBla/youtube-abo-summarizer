...
- **Read/bookmark tracking**: Each video card has read and bookmark buttons; state is persisted in browser cookies (365 days) and shared between the report and export views. In the export, the read/bookmark/share buttons repeat at the foot of an expanded summary — together with a collapse button — so a long text does not have to be scrolled back up to be marked or closed
...
`export.py` renders all (or a subset of) stored videos into a single self-contained HTML file for offline browsing. It includes client-side search across titles and summaries, a "published after" date filter, channel/tag/read/bookmark filter dropdowns and a length filter (under 5/10/20/30/60 minutes), each with a descriptive label, sorting by publish date / date added / channel / title, and pagination (20 items per page). The "Zuletzt hinzugefügt" ("Recently added") sort uses the store's `collected_at` timestamp, so videos queued through the Ingest button appear at the top regardless of when they were published. Tag chips on each video card are clickable and set the tag filter directly. The length filter cuts strictly — "< 5 Min" leaves out a video of exactly 5:00 — and videos whose duration was never recorded stay out of it, visible only under "Alle". Read and bookmark state is tracked in browser `localStorage` and persists across sessions. No server required.
...
`--exclude-channel` leaves a channel out of the book — give its ID or its exact name (case-insensitive), comma-separated and repeatable, e.g. `--exclude-channel "The Daily Show" --exclude-channel UCskip,Musikloops`. The exclusion runs before `--limit`, so the limit counts the chapters that remain, and a run that excludes everything exits without writing a book. A standing list can live in `cron.env` as `EBOOK_EXCLUDE_CHANNELS="The Daily Show,UCskip"` instead of in every cron line — it is read by `ebook.py` only, so the HTML export and the mail digests keep showing every channel, and `--exclude-channel` adds to it rather than replacing it. An `EBOOK_EXCLUDE_CHANNELS` variable already present in the environment wins over the file.
...
`ebook.py` renders stored videos into a single EPUB 3 file for reading offline on an e-reader or the Kindle app — one chapter per video, most recently added first, grouped by ISO calendar week in the table of contents. Each chapter's meta line names the channel, the release date **and time** (HH:MM, converted from the store's UTC stamp to the local zone of the machine building the book) and the running time, each with its summary, thumbnail (optional) and transcript (optional). Every video is its own document, so the reader's next-chapter jump moves video by video and each one starts on a fresh page; a small line above the title names the calendar week it belongs to. Like `export.py`, it only reads from `data/`: no YouTube API calls and no LLM calls. The title page shows the book's covered date range (earliest to latest video) alongside the video counts and generation date; a video whose transcript couldn't be fetched shows the matching localized notice instead of an empty section.
