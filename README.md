...
| `--fix-links` | Repair broken markup in stored summaries: summaries stored as a JSON object, unclosed hrefs that swallow whole sentences, links showing a period instead of their timestamp, wrong `t=` offsets, anchors the model left open, missing `ts-link` class, `</p>` tags closed mid-sentence. No LLM calls — back up `data/summaries/` first, the rewrite is in place |
