...
while IFS= read -r line; do
    ...
    # collect.py exits with EXIT_NEW_VIDEOS (10) when it stored the video, or
    # when it wrote the summary a stored-but-incomplete entry was missing --
    # a success in both cases, not a failure. Anything else non-zero is a real
    # error.
    if [ "$rc" -eq 0 ] || [ "$rc" -eq "$EXIT_NEW_VIDEOS" ]; then
        success=1
        echo "[$(date -Iseconds)] done $video_id (exit $rc)" >> "$LOG"
    else
        next=$((attempts + 1))
