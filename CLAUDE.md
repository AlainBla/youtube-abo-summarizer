...
| `export.py` | Export CLI (also writes the `<output>.meta.json` update manifest via `renderer`): renders a self-contained HTML archive with client-side search, channel/tag/read/bookmark/length filters, sort (publish date, date added, channel, title), and pagination; passes each video's `collected_at` into the embedded index so the "date added" sort works; `--thumbnail` for static images, `--sync-url` to embed the sync server, `--show-model` for LLM badge, `--no-compress` to embed data uncompressed; `--user EMAIL` renders that sync user's personal view (`filter_personal()`) and writes the unfiltered archive beside it at `full_sidecar_path()` (`yt.html` → `yt.full.html`), which the filtered page links to |
