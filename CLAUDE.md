...
- `--no-proxy` ignores every configured proxy — `SOCKS_PROXY_URL` and `WEBSHARE_PROXY_URL` alike — and fetches via a direct connection.
- Proxy order is one chain, built by `proxies.proxy_chain()` and shared by `transcripts.py`, `feeds.py` and `ytdlp_meta.py`: a **SOCKS proxy first**, the Webshare residential proxy behind it. `SOCKS_PROXY_URL` is not trusted, it is probed — a TCP connect plus, for SOCKS5, the greeting handshake, so a tunnel that is configured but not running (Tor stopped, the `ssh -D` session gone) is skipped instead of failing every fetch, and an unrelated service holding the port is not mistaken for a proxy. The verdict is cached per process and logged once. `feeds.py` and `ytdlp_meta.py` still try a direct connection **before** the chain (a healthy IP should not pay a proxy's latency); `transcripts.py` does not, because YouTube blocks a server's own IP for transcript requests far too readily — there the chain's first entry *is* the first attempt.
...
The manifest cannot say *which* video an export brought — an ingested backfill is not `newest_id` — so a changed `generated_at` only triggers a reload into `?v=ID`, and the deep-link view decides: `ingestWaitObserve(id, found)` clears the wait when the video is on screen and keeps it when the export belonged to something else. Most reloads are that second case, so the deep-link view distinguishes it in what it prints: `singleMissingKey(videoId, saved, now)` (pure, unit-tested) answers `waiting` while a live wait names this very video, and `singleMissingHtml()` then shows `singleWaiting` — "ist eingereiht … Seite lädt sich neu, sobald sie da ist" — instead of `singleNotFound` plus the link into the full archive, which does not have the video either and reads as a failure where the truth is "not yet". An **expired** wait is not a wait: there the archive really is the last word and the old wording returns. The key is read once, at render time, so `refreshSingleMissingMessage()` re-renders the deep-link view from `cancelIngestWait()`, the poll's timeout branch and `startIngestWait()` ("weiter warten") — otherwise the page goes on promising a reload nobody will send, or calls the video missing while a fresh wait runs. It is deliberately not called from `clearIngestWait()`/`saveIngestWait()`: `renderPage()` calls `ingestWaitObserve()`, which clears the wait, and that would recurse. `resumeIngestWait()` re-pins `generated_at` to the manifest of the page it is running on **before** polling resumes; without that the saved value is older than the current export by definition and every poll would reload again, forever. `tests/test_export_ingest_wait.py` pins that specifically.
...
On the userscript side that shortcut is visible in the verdict, which the page states outright: `setIngestVerdict()` writes `data-ingest-id` and `data-ingest-result` (`queued` / `already` / `failed` / `logged-out`) onto `#sync-ingest`, cleared at the top of every `doIngest()` so a previous submission's answer can never be read as this one's. On `already` the relay opens the archive at `?v=ID` in a foreground tab, since the background tab deliberately does not navigate — which is why the verdict has to be right: a `queued` video reported as `already` sends the reader to "Video … ist nicht in diesem Archiv" on a video that was only just queued. That is what the pre-1.8.0 reading produced (see the userscript section).
...
The verdict comes from the archive page itself, never off `#sync-status`: that line is localised *and* written by the login and sync handlers, so a change there right after the Ingest box appears may belong to something else entirely. The page states the outcome on `#sync-ingest` as `data-ingest-id` plus `data-ingest-result`, and `ingestVerdict(dataset, videoId)` (pure, pinned by `tests/test_userscript.py`) reads it, ignoring an id that is not this submission's. Until v1.8.0 the script inferred it from `doIngest()`'s own mechanics instead — the button's `disabled` flag while the request is in flight, and the input, which is cleared only on 200/202 — and that was a race it usually lost: `postIngest()` re-enables the button *before* clearing the input, so a fast 202 flipped the flag back between two 250 ms polls, and a cleared input never seen disabled read as `already`. In the background tab the script opens, where timers are clamped to a second, nearly every queued video was reported that way. `speaksVerdict()` tests for the marker `data-ingest-result=""` in the markup, so an archive exported before v1.8.0 still gets the old reading (`legacyVerdict()`) rather than no verdict at all. A timeout waiting for `#sync-ingest` to become visible is reported as "not logged in", which is what it almost always is. Success is relayed back to the YouTube tab through `GM_setValue`/`GM_addValueChangeListener`, so the button itself confirms.
...
| `proxies.py` | The one place that decides which proxy to use: `socks_proxy_url()` reads `SOCKS_PROXY_URL`, normalises it (a bare `host:port` becomes `socks5h://host:port`, missing port → 1080), and **probes** it — `socket.create_connection` plus the SOCKS5 method-selection handshake, so neither a dead tunnel nor an unrelated listener is handed to a caller. A missing PySocks counts as unusable too (`_pysocks_available()`): `requests` answers a `socks5://` proxy with `InvalidSchema`, a `ValueError` that neither `youtube-transcript-api` nor `_fetch_original()` catches, so an answering tunnel without the package would end a collect run in a traceback instead of one failed video. The whole verdict — parsed, PySocks present, port answering — the verdict is cached per normalised URL is cached under the raw environment value (`reset_probe_cache()` for tests) and printed once, so a misconfiguration is not reported per video. `webshare_proxy_url()` is never probed (metered, remote). `proxy_chain(no_proxy)` returns `[socks?, webshare?]` in that order, empty under `--no-proxy`; `redact()`/`describe()` keep passwords out of the log |
| `feeds.py` | Quota-free channel discovery through `youtube.com/feeds/videos.xml?channel_id=UC…`: `get_new_videos_rss()` returns videos published after `since` in the same four-key shape as `youtube_client.get_new_videos()`, or `None` when it cannot answer — fetch failed, feed empty or unparseable, or every one of its ~15 entries lies inside the window, where older ones may have been cut off (an empty list is a real "nothing new"). `<published>` only, never `<updated>`: an edited description would otherwise resurrect a years-old video as new. `get_channel()` reads a channel's title from its feed, so a `UC…` identifier needs no `channels().list`. Direct fetch first, then every proxy in `proxies.proxy_chain()` in turn (SOCKS before Webshare); a SOCKS attempt goes through `requests` (`_fetch_socks()`), because `urllib`'s `ProxyHandler` cannot speak SOCKS at all; thumbnails normalised to `mqdefault.jpg`, matching the API path and `ytdlp_meta` |
...
| `ytdlp_meta.py` | Quota-free single-video metadata via yt-dlp (`get_video_metadata()`): runs `python -m yt_dlp --dump-single-json` on the watch URL in a subprocess — no OAuth token, no API key, no quota — and returns the same dict shape as `youtube_client.get_video_by_id()`, or `None` on any failure (never raises, because the caller's whole point is the fallback). Direct fetch first, then every proxy in `proxies.proxy_chain()` in turn (SOCKS before Webshare — yt-dlp speaks `socks5h://` natively via `--proxy`), because the server's own IP is the blocked one; seconds → ISO 8601 duration, `release_timestamp`/`timestamp` → `published_at` with `upload_date` (midnight UTC) as last resort; the thumbnail URL is built as `i.ytimg.com/vi/<id>/mqdefault.jpg` rather than taken from yt-dlp's `.webp` field, which `ebook.collect_thumbnails()` could not use |
| `transcripts.py` | `youtube-transcript-api` wrapper; language priority via `TRANSCRIPT_LANGS` (default: de,en); handles ip_blocked / rate_limited / country_blocked errors; the **first** attempt already goes through the best proxy `proxies.proxy_chain()` offers (SOCKS when one answers, else Webshare, else direct) — unlike `feeds.py`/`ytdlp_meta.py` there is no direct-first step; on `ip_blocked` it works down the rest of the chain and finally the country-pinned Webshare URL (`_blocked_retry_urls()`); API objects are built lazily and cached per proxy URL (`_api_for()`), so importing the module never probes a socket; `VideoUnplayable` is only classified as `country_blocked` when the reason mentions "country"/"region" — on `country_blocked`, retries once with a country-pinned Webshare proxy (`PROXY_FALLBACK_COUNTRY`, default: DE) if `WEBSHARE_PROXY_URL` is set — `_country_proxy_url()` is guarded by scheme, so the `-DE` username trick is never applied to a SOCKS URL, which has no such convention; other `VideoUnplayable` causes fall to `unavailable` (retryable); `requests.exceptions.ProxyError` and `ConnectionError` are caught and mapped to `unavailable`; logs proxy configuration on startup |
...
| `userscript/yt-ingest.user.js` | Violentmonkey/Tampermonkey script: a button on YouTube that queues the current video, by driving the Ingest field of the logged-in archive page instead of holding a token of its own; `speaksVerdict()`/`ingestVerdict()`/`legacyVerdict()` read the outcome off the page's `data-ingest-*` attributes, falling back to the pre-1.8.0 inference on an archive exported before them. Two host-specific lines (`ARCHIVE_URL`, the archive `@match`) |
...
# ── Optional: local SOCKS proxy, tried before the residential one ─────────────
# Tor, an `ssh -D` tunnel, a VPN's own listener. Used only when the port answers
# a SOCKS handshake. A bare "host:port" is read as socks5h://host:port, and
# socks5h keeps DNS on the proxy side. Needs PySocks (in requirements.txt).
# SOCKS_PROXY_URL=socks5h://127.0.0.1:9050
# Seconds to wait for the probe (default: 2).
# SOCKS_PROBE_TIMEOUT=2
