...
- `select_videos()` filters (by `--channel`/`--videos`/`--tag`) and sorts newest-first; `main()` then applies the read filter and only afterwards cuts to `--limit` (default `DEFAULT_LIMIT` = 100, `0` = uncapped). The limit therefore counts the videos that actually reach the book: `--limit 50 --read drop` means the 50 newest unread ones. Chapter files are keyed by part **and** week (`chapter-<part>-<anchor>.xhtml`) because `--read split` puts the same calendar week in both parts — keying on the week alone made the second part overwrite the first and silently drop its videos.
