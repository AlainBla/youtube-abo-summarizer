...
- `select_videos()` filters (by `--channel`/`--videos`/`--tag`) and sorts newest-first; `main()` then applies the read filter and only afterwards cuts to `--limit` (default `DEFAULT_LIMIT` = 100, `0` = uncapped). The limit therefore counts the videos that actually reach the book: `--limit 50 --read drop` means the 50 newest unread ones. Each video is its own chapter document (`video-<video_id>.xhtml`, one spine item each), so a reader's next-chapter jump moves video by video; weeks stay the grouping level in nav and NCX, and a kicker line above each title names the week. A week's NCX navPoint points at its first video and must therefore carry that video's `playOrder` — differing values for one target are an NCX violation.
