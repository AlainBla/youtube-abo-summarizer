...
`export.py` renders stored videos into a self-contained HTML file for offline browsing (client-side search, channel/tag/read/bookmark filters, sort, pagination; read and bookmark state persisted in browser `localStorage`).
...
Embedded data is split into a lightweight search index (metadata + a precomputed lowercased `search_text` field; drives filter/sort/dropdowns/search) and a separate `summaries` map (video_id → summary HTML, consulted only when a card renders). The combined `{index, summaries}` JSON is gzip+base64 embedded and decompressed in-browser via the native `DecompressionStream('gzip')` API (Chrome 80+/FF 113+/Safari 16.4+); the UI bootstraps asynchronously once the blob is decoded. `--no-compress` embeds the JSON as a plain JS object literal instead (larger file, no `DecompressionStream` needed). Preview iframes are click-to-load facades (thumbnail + play button) that swap in the YouTube embed only on click.
...
| `export.py` | Export CLI: renders a self-contained HTML archive with client-side search, channel/tag/read/bookmark filters, sort, and pagination; `--thumbnail` for static images, `--sync-url` to embed the sync server, `--show-model` for LLM badge, `--no-compress` to embed data uncompressed |
...
| `renderer.py` | Jinja2 renderer; writes the final HTML file; accepts `lang=` kwarg; sanitizes summaries at render time via `_sanitize_summary()` — strips any trailing incomplete HTML tag to guard against LLM output truncated mid-tag (which would cause the browser to consume subsequent cards as an attribute value); `render_export_html()` splits export data into a lightweight `index` (with `search_text` via `_strip_html_to_text()`) and a `summaries` map, then gzip+base64 embeds the JSON (`compress=True`, default) or embeds it as a plain JS object literal (`compress=False`) |
