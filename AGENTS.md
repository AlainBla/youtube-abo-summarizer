...
- **A chapter file is keyed by part *and* week.** With `--read split` the same calendar week appears in both parts; `chapter-<anchor>.xhtml` alone made the second write overwrite the first, so those videos vanished from the book without a word. `tests/test_epub_structure.py::test_the_same_week_in_two_parts_gets_two_chapter_files` guards it, and the OPF spine derives its idref via `item_id(week.href)` rather than rebuilding the name.
