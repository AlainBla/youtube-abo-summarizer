...
- `renderer._export_manifest(index, chunks)` carries `summary_count` and `summary_digest` besides the counts; the page's `updateBannerKey(current, pending)` may only read them when the video-count delta is exactly 0 and *both* manifests carry the fields — a shrinking export changes the digest because videos left, and an archive predating the fields would otherwise be told its summaries changed
